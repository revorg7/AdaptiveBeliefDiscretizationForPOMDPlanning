...
#        cond = True
#        while cond:
#          vals = self.decode_rocks(np.random.random_integers(0, (1 << self.n_rocks) - 1))
#          n = sum(x > 0 for x in vals)
#          if n == int (self.n_rocks / 2.0) + 1:
#            cond = False
        return np.random.binomial(1,0.5,self.n_rocks)
...
#                                                        ^^DIVYA: Since this is essentialy an observation (obs overwrites true-state in makeobs fn smhow, idk why he does that)
#                                                    ^^ DIVYA NEWER: What he did is actually correct, otheriwise thr is an information leak and obs-actions are rendered useless
...
#                    # "next states".rock_states[rock_no] is set to False in make_next_state
                ...
                if self.actual_rock_states[rock_no] and state.rock_states[rock_no] and (rock_no not in self.unique_rocks_sampled):
